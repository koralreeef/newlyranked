...
const { buildFull, buildEmbed } = require('../seasonlbHelper.js')
const { currentSeasons, seasonScores, osuUsers } = require('../db/dbObjects.js');
const { currentD1Collection, currentD2Collection } = require('../config.json');
const Sequelize = require('sequelize');
const Op = Sequelize.Op;
const regex = /^\.seasonlbs/gm;
...
module.exports = {
  ...
  async execute(message) {
    ...
    if (regex.test(msg.substring(0, 11))) {
      ...
      let division = 1;
      let aimList = await currentSeasons.findAll({
      ...
      //VERY BAD LMAO
      console.log(msg.substring(11, 12))
      if (msg.substring(11, 12) == "2") {
        division = 2;
        aimList = await currentSeasons.findAll({
          where: { division: division },
          order: [
            ["map_id", "DESC"],
          ]
      ...
      //THIS SUCKSSSSSS
      let brah = await seasonScores.findAll({
      ...
      collector.on("collect", async (m) => {
        ...
        let magnifyUnfiltered = await seasonScores.findAll({
          where: { map_id: aimList[ind].map_id },
        })
        if (mod != "") {
          magnifyUnfiltered = await seasonScores.findAll({
            where: { map_id: aimList[ind].map_id, mods: modsCounted },
          })
        }
        //extra query :(((((((())))))))
        const unique = []
        ...
        if (m.customId === "nm" + epoch) {
          ...
          mod = '+NM'
          modsCounted = 0;
          magnifyMapCount = await seasonScores.count({
            where: { map_id: aimList[ind].map_id, required_mods: modsCounted },
          })
          if (magnifyMapCount <= 15) {
            magnifyForward.setDisabled(true);
            magnifyBackward.setDisabled(true);
